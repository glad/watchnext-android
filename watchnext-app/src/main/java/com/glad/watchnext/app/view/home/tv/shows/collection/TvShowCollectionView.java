 ...
 */
public final class TvShowCollectionView extends FrameLayout implements TvShowCollectionContract.View {
    @NonNull private static final String TAG = LogUtil.getTag(TvShowCollectionView.class);

    @Inject TvShowCollectionContract.Presenter presenter;
    ...
    //region Constructor
    public TvShowCollectionView(@NonNull final Context context) {
        this(context, null);
    }

    public TvShowCollectionView(@NonNull final Context context, @Nullable final AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TvShowCollectionView(@NonNull final Context context, @Nullable final AttributeSet attrs, final int defStyleAttr) {
        this(context, attrs, defStyleAttr, 0);
    }

    public TvShowCollectionView(@NonNull final Context context, @Nullable final AttributeSet attrs, final int defStyleAttr, final int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        Injector.INSTANCE.inject(this);
        inflate(context, R.layout.layout_view_collection, this);
        ButterKnife.bind(this);
        viewAdapter = new CollectionViewAdapter(recyclerView, imageProvider, R.drawable.ic_tvshow_poster_placeholder);
    ...
    public void onCreated(@NonNull final Serializable... args) {
        ...
        isListViewSubject.subscribe(isListView -> {
            viewAdapter.updateView(getContext(), isListView ?
    ...
    //region TvShowCollectionContract.View
    @Override
    ...
    @Override
    public void onLoaded(@NonNull final TvShowSimplifiedPresentationModel tvshow) {
        log.d(TAG, "onLoaded() called with: tvshow = [" + tvshow + "]");
        viewAdapter.add(CollectionViewModel.newBuilder()
                .id(tvshow.getId())
                .title(tvshow.getName())
                .body(tvshow.getOverview())
                .imageUrl(tvshow.getPosterImageUrl())
                .build());
    }
    ...
    //endregion TvShowCollectionContract.View
}